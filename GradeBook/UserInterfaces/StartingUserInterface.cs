...
namespace GradeBook.UserInterfaces
{
    public static class StartingUserInterface
    {
        ...
        public static void CreateCommand(string command)
        {
            ...
            var name = parts[1];
            var type = parts[2];
            var IsWeigth = Convert.ToBoolean(parts[3]);
            if (type == "standard")
            {
                StandardGradeBook sGradeBook = new StandardGradeBook(name, IsWeigth);
                Console.WriteLine($"Created gradebook {name}");
                GradeBookUserInterface.CommandLoop(sGradeBook);
            }
            else if (type == "ranked")
            {
                RankedGradeBook rGradeBook = new RankedGradeBook(name, IsWeigth);
                Console.WriteLine($"Created gradebook {name}");
